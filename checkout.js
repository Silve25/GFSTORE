...
(() => {
  ...
  function updateSummary(){
    ...
    if(currentPM()==='crypto'){
      ...
      const fx={BTC:{rate:60000,symbol:'BTC'},'ETH-ERC20':{rate:3000,symbol:'ETH'},'USDT-TRC20':{rate:1,symbol:'USDT'},'USDT-BEP20':{rate:1,symbol:'USDT'}};
      ...
      const wallets={
        'USDT-BEP20':{address:'0x5f1e4fdef890dba03ebfcba79a77aa0ea432f04b',qr:''},
        'USDT-TRC20':{address:'TMdLMTYGEEZAnoVbi482Zr9QSRgWmRUXXq',qr:''},
        'BTC':{address:'18npEW9EaKZvVvA9B1z3DcmVEJtuQUdus3',qr:''},
        'ETH-ERC20':{address:'0x5f1e4fdef890dba03ebfcba79a77aa0ea432f04b',qr:''}
      };
      const w = wallets[ccy]||{};
      const wEl=$('#wallet'); if(wEl) wEl.value=w.address||'';
  ...
  $$('.m[role="tab"]').forEach(card=>{
    card.addEventListener('click', ()=>{
      ...
      bankVerified=false; cryptoVerified=false;
      updateCTA(); updateSummary();
    });
  });
  ...
  once($('#confirmBank'), async ()=>{
    ...
    if(!$('#vref')?.value.trim()){
      ...
      const base=(name||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toUpperCase().replace(/[^A-Z0-9]+/g,'').slice(0,16) || orderId.replace('GF-','').split('-')[0];
      $('#vref').value = base;
    }
  ...
  (function init(){
    ...
    $('#ccy')?.addEventListener('change',()=> updateSummary());
